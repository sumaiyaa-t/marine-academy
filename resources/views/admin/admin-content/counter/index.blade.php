...
@section('content')
    ...
    <table id="example" class="display" style="width:100%">
        ...
        <?php $id = 0 ?>
        @foreach($counters as $testimonial)
            <tr>
                <td>{{ $id += 1 }}</td>
                ...
                <td><a href="{{ route('counter.edit',$testimonial->id) }}" class="btn btn-info">Update</a></td>
                <td>
                    <form action="{{ route('counter.destroy',$testimonial->id) }}" method="post">
                        {{ csrf_field() }}
                        @method('delete')
